...
import com.jetbrains.FileDialog;
...
public class FileDialogTest {
    ...
    public static void main(String[] args) throws Exception {
        directory.mkdirs();
        refresh();
        var dialog = new java.awt.FileDialog((Frame) null);
        dialog.setTitle("Press Save");
        dialog.setDirectory(directory.getAbsolutePath());
        dialog.setFile("PASS");
        dialog.setMode(java.awt.FileDialog.SAVE);
        ...
        file.createNewFile();
        dialog = open(FileDialog.SELECT_FILES_HINT);
        dialog.setVisible(true);
        check(dialog, file);

        refresh();
        file.mkdir();
        dialog = open(FileDialog.SELECT_DIRECTORIES_HINT);
        dialog.setVisible(true);
        check(dialog, file);
        ...
        file2.mkdir();
        dialog = open(FileDialog.SELECT_FILES_HINT | FileDialog.SELECT_DIRECTORIES_HINT);
        ...
        dialog = open(FileDialog.SELECT_DIRECTORIES_HINT | FileDialog.CREATE_DIRECTORIES_HINT);
        dialog.setVisible(true);
    ...
    private static java.awt.FileDialog open(int hints) {
        var dialog = new java.awt.FileDialog((Frame) null);
        dialog.setMode(java.awt.FileDialog.LOAD);
        FileDialog.get(dialog).setHints(hints);
        return dialog;
    }

    private static void check(java.awt.FileDialog dialog, File... expected) {
